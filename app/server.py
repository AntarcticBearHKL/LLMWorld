...
def load_worlds_matrix():
    ...
    path = os.path.join(SIMULATION_DIR, "comparison", "worlds_matrix.json")
    if not os.path.exists(path):
        return None
    with open(path, "r", encoding="utf-8") as f:
        return json.load(f)
...
def delete_world(world_id):
    ...
    for target in (os.path.join(WORLDS_DIR, world_id),
                   os.path.join(SIMULATION_DIR, world_id)):
        ...
        try:
            ...
            if len(parts) >= 3 and parts[:2] == ["api", "worlds"]:
                ...
                if parts[3] == "seasonal" and len(parts) >= 5:
                    ...
                    return
                if parts[3] == "weather" and len(parts) >= 5:
                    data = load_weather_sensitivity(world_id, parts[4])
                    self._send_json(data or {"error": "no weather"}, 200 if data else 404)
                    return
        ...
        try:
            ...
            if len(parts) == 4 and parts[:2] == ["api", "worlds"] and parts[3] == "simulate":
                ...
                try:
                    ...
                    runner = os.path.join(PROJECT_ROOT, "src", "population_runner.py")
                    proc = subprocess.run([python, runner] + sim_args,
                                          capture_output=True, text=True, timeout=7200)
                    if proc.returncode != 0:
                        raise RuntimeError((proc.stderr or proc.stdout)[-800:])
                    self._send_json({"ok": True, "command": " ".join(sim_args)})
