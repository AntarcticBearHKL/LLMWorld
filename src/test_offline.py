...
IMPLEMENT_DIR = os.path.join(PROJECT_ROOT, "src")
...
class TestGroupAnalysis(unittest.TestCase):
    ...
    def test_load_variability_labels_mapping(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            import os
            real_path = os.path.join("simulation", "__fake_world", "analysis",
...
class TestServerData(unittest.TestCase):
    _OUT = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "simulation")
...
class TestCompareWorlds(unittest.TestCase):
    ...
    def _write_matrix(self, world_id, scenarios):
        import os
        path = os.path.join("simulation", world_id, "comparison",
                            "policy_matrix.json")
    ...
    def test_build_matrix_shape(self):
        ...
        try:
            ...
            for w in ("__cw_a", "__cw_b"):
                path = os.path.join("simulation", w)
...
class TestTimelineServer(unittest.TestCase):
    ...
    def test_load_timeline_merges(self):
        ...
        import os, shutil
        base = os.path.join("simulation", "__tl_world")
...
class TestWorldSummary(unittest.TestCase):
    ...
    def _write(self, world_id, filename, data):
        import os
        path = os.path.join("simulation", world_id, "analysis", filename)
        os.makedirs(os.path.dirname(path), exist_ok=True)
        with open(path, "w", encoding="utf-8") as f:
            json.dump(data, f)
    ...
    def test_summary_merges(self):
        ...
        try:
            ...
            shutil.rmtree(os.path.join("simulation", "__ws"), ignore_errors=True)
...
class TestPolicyTradeoffs(unittest.TestCase):
    ...
    def test_build_report(self):
        ...
        import os, shutil
        path = os.path.join("simulation", "__pt", "comparison", "policy_matrix.json")
        os.makedirs(os.path.dirname(path), exist_ok=True)
        with open(path, "w", encoding="utf-8") as f:
            json.dump({"scenarios": [
        ...
        try:
            ...
            shutil.rmtree(os.path.join("simulation", "__pt"), ignore_errors=True)
...
class TestApplianceUsage(unittest.TestCase):
    ...
    def test_build_report(self):
        ...
        import os, shutil
        base = os.path.join("simulation", "__au", "3168", "h1", "baseline",
                            "20260501", "用电信息")
        ...
        try:
            ...
            shutil.rmtree(os.path.join("simulation", "__au"), ignore_errors=True)
...
class TestExportCsv(unittest.TestCase):
    ...
    def test_export(self):
        ...
        import os, shutil
        analysis = os.path.join("simulation", "__csv", "analysis")
        ...
        try:
            ...
            shutil.rmtree(os.path.join("simulation", "__csv"), ignore_errors=True)
