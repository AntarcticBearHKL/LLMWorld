...
class Dishwasher(CycleAppliance):
    def __init__(self, brand=None, power=None, energy_per_cycle_kwh=None, cycle_minutes=None, age=0,
        ...
        power_watts = power if power is not None else 1800
        cycle_kwh = energy_per_cycle_kwh if energy_per_cycle_kwh is not None else 1.1
        minutes = cycle_minutes if cycle_minutes is not None else 120
        super().__init__("Dishwasher", power_watts, cycle_kwh, minutes, brand=brand, age=age,
    ...
    def get_config_schema(cls):
        return {
            "type": "Dishwasher",
            "description": "Dishwasher (cycle appliance)",
            "config_fields": {
                "brand": {
                    "type": "string",
