# -*- coding: utf-8 -*-
"""PersonaReader - read MatrAIx synthetic persona chunked CSVs.

Usage:
    from reader import PersonaReader
    reader = PersonaReader()                       # auto-detect CSV chunks next to this file
    rows = reader.load_chunk(1)                    # first chunk, list[dict]
...
class PersonaReader:
    ...
    def load_chunk(self, n):
        """Load one chunk (1-based index). Returns list[dict] keyed by column label."""
    ...
    def filter(self, conditions, chunks=None):
        """conditions: dict {column_label: value} - all must match (exact string).
        chunks: optional iterable of 1-based chunk numbers to search (default all)."""
        out = []
