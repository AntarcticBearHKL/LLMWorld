...
- **阻塞（结构性 null）**：`world_143345` 的 EV 充电 **100% 谷期**；且剥离 overnight 提示后
  （`--natural-ev`）**仍 100% 谷期**（R080）→ "夜间充电"是 agent 的**内在默认**，非 prompt 造成。
  故 `subsidy`/TOU/`ev_delay` 的 EV 峰移干预在本平台**不可评估**。
