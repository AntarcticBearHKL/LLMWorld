...
> 状态：**定稿（有效果）**：cold_snap 使采暖设备由全 0 变为启用；
> 跨 **10 个成员-日** baseline **0/10** vs cold_snap **6/10**：Fisher **双侧 p≈0.011（显著）**，跨 3 世界。
> 数值来自 R062–R065/R072/R093。
...
| w143 h002M1 | 0.000 | 12.200 | ✓ |
| w143 h002M2 | 0.000 | 1.500 | ✓ |

汇总：baseline **0/10** vs cold_snap **6/10**。Fisher **双侧 p ≈ 0.011（显著）**（R093）——
跨 3 世界；启用率 60%，仍低于热浪的 ~92%。
...
- **显著**：cold_snap 使居民启用采暖；baseline **0/10** vs cold_snap **6/10**，Fisher **双侧 p≈0.011**（跨 3 世界）。
  效应大（1.5–12 kWh）但启用率（60%）仍低于热浪（~92%）——**事件不同、稳健性不同**。
- **可复现命令**：见第 3 节。
