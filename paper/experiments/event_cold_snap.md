...
**跨户样本（R063；采暖 = SpaceHeater + AirConditioner，>0 记为启用）**
...
汇总：baseline **0/3** vs cold_snap **2/3**（Fisher 单侧 **p = 0.20，未显著**）——
**方向一致但异质**（启用概率低于热浪），需更多样本。
...
- **初步（未显著）**：cold_snap 可使 LLM 居民启用采暖（单户 +158%），但跨户启用率仅 **2/3**
  （p=0.20）——**大效应但异质**，与热浪（9/10, p≈0.0004）相比稳健性较低。
- **下一步**：扩样（更多含采暖设备的住宅）方能判定。
