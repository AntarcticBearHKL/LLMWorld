...
> 状态：**初步（单侧边缘；双侧未显著）**：cold_snap 使采暖设备由全 0 变为启用（单户 +158%）；
> 跨 8 个成员-日 baseline **0/8** vs cold_snap **4/8**：Fisher **单侧 p≈0.038**、**双侧 p≈0.077（未达 0.05）**。
> 数值来自 R062–R065/R072；须更多样本。
...
汇总：baseline **0/8** vs cold_snap **4/8**。Fisher **单侧 p ≈ 0.038**，**双侧 p ≈ 0.077（未达 0.05）**
（R072 精确复核）——方向一致、效应大，但**证据不足**；启用率 50%，明显低于热浪的 90%。
...
- **方向一致，双侧未显著**：cold_snap 使居民启用采暖；baseline **0/8** vs cold_snap **4/8**，
  Fisher **单侧 p≈0.038、双侧 p≈0.077**（R072）。效应大（9–12 kWh）但启用率（50%）低于热浪（90%）。
- **可复现命令**：见第 3 节。
- **下一步**：更多样本方能在双侧检验下判定。
