...
> 状态：**初步有效果（单成员探针，待复现）**。数值来自 R016；在多人户/多日/多种子复现前，
> 不应作为最终论文结论。
...
- 09-11 总量较对照 **+64.2%**，AC 占比 ≈45%；09-12 总量 **+17.1%**；
- AC 由**全程 idle** 变为**实际 use**（事件-天气联动生效后）。
...
- **初步**：以自然语言注入的热浪事件可使 LLM 居民自发启用空调、抬升日总电量（单成员 +64%），
  方向与文献一致；前提是新闻与环境天气**一致**。
- **可复现命令**：见第 3 节。
- **下一步**：多人户 × 多日 × 多种子复现，并用 `analyze_event_response.py` 度量事件日模式转移。
