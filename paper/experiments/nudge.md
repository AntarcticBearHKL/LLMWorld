...
**方向全部为负（−5.8%~−23.3%），多成员稳健；幅度均超 −1~3% 基准。**
...
- **初步（方向多成员稳健）**：固定社会规范 nudge（含规范性措辞）在**2 住户 / 5 成员-日**上一致降低总用电
  （−5.8%~−23.3%），方向稳健但幅度均超 −1~3% 基准；R040 去指令消融（`nudge_soft`）使效应消失（≈0）
  → 效应由**规范性措辞**驱动，支持 **prompt 过度遵从**。
- **可复现命令**：见第 3 节（含 `--policy nudge_soft` 消融）。
- **下一步**：用 `analyze_groups.py` 看个体异质性是否与 awareness/尽责性相关。
