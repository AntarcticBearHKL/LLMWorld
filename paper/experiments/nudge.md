...
> 状态：**方向不稳定**：R054 扩大样本（新世界 7 名成员）后 **3 人反而增耗**、组内离散度压倒组间差 →
> nudge 效应**不稳健**；R040 去指令消融曾显示效应由规范性措辞驱动。数值来自 R037/R040/R043/R048/R053/R054。
...
**Multi-member paired samples — house_0001 (R043/R048)**
...
全样本（2 户、5 成员-日）：house_0002 −10.29% / −5.76%；house_0001 均值 ≈−17.4%。
**这 5 个样本全为负——但见下方 R054 的反证。**

**Larger sample on the fresh world — contradicts the above (R054)**

world_172148，`nudge` vs baseline（1 天）：

| 住户 | 成员 | awareness | 变化 |
|---|---|---|---|
| house_0001 | M1 | Low | −25.26% |
| house_0001 | M2 | Medium | **+8.55%** |
| house_0001 | M3 | Low | **+39.42%** |
| house_0001 | M4 | Medium | **−45.13%** |
| house_0001 | M5 | Low | **+9.69%** |
| house_0002 | M1 | Low | −10.10% |
| house_0002 | M2 | Medium | −17.41% |

**7 人中 3 人增耗**；组内离散度（±25~45pp）**远超**任何均值差 →
**nudge 的降耗方向在小样本上稳健，但在更大样本上并不稳健**。此前"5/5 全负"是小样本巧合。
...
- **方向不稳健**：小样本（5/5 负）看似一致，但新世界大样本（7 人）中 **3 人增耗**、离散度压倒均值差 →
  不能声称 nudge 稳定降耗。R040 消融仍提示效应与**规范性措辞**相关。
- **可复现命令**：见第 3 节（含 `--policy nudge_soft` 消融）。
- **下一步**：大规模多种子平均（当前唯一可信路径）。
