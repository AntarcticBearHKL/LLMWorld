...
> 状态：**定稿（有效果）**：lockdown 使外出行程 **13/13 → 1/13** 归零（Fisher 双侧 p≈0.000003，跨 3 世界），
> 并把日间(9–17)负荷抬升（**same-era n=9: +220.7%, t=3.34, p≈0.010**；Out=0 0/9→7/9）。
> 数值来自 R068/R069/R086/R088/R132。
...
**日间负荷幅度（R088，13 对配对；⚠️ 跨时段，已被下方 R132 same-era 取代为**主证据**）**：日间(9–17)均值 **1.783 → 3.966 kWh（+122%）**，
配对差 **+2.183 kWh**（std 0.979，**13/13 为正**，paired t≈**8.04**）——效应约为配对 std 的 2.2×，
**远超噪声地板**（R057, std≈1.0 kWh），是本工作**可判的幅度结论**。

**Same-era 验证（R107）**：fresh 基线 0.680 kWh → 同期封锁 **3.257 kWh（+379%）**；
且日间基线**未漂移**（旧 0.649 ≈ fresh 0.680）→ **该幅度真实**（不同于峰段漂移伪影）。

**Same-era 交错 n=9（R132，有效设计）**：`world_838587` h002 M1，交错 baseline×9 vs lockdown×9：

| 指标 | baseline | lockdown | Δ | paired t(df=8) |
|---|---|---|---|---|
| **日间(9–17) kWh** | 0.649 ± 0.000 | 2.081 ± 1.288 | **+220.7%** | **+3.34（p≈0.010）** |
| 外出 min | 598 ± 5 | 133 ± 265 | −77.7% | −5.29 |
| Out=0（完全居家） | **0/9** | **7/9** | — | **Fisher p≈0.002** |

→ **日间幅度 + Out 归零均以有效设计（same-era, n=9）确证**。
...
- **显著且最强**：lockdown 使户外时间近乎归零（baseline **13/13** → lockdown **1/13**, 双侧 p≈0.000003，
  跨 3 世界；same-era n=9 复现 0/9→7/9），并把**日间(9–17)负荷抬升**——
  **same-era 交错 n=9 定稿：+220.7%（t=3.34, p≈0.010）**——是本项工作**效应量最大且经有效设计量化**的结论。
- **可复现命令**：见第 3 节（same-era：交错跑 `--env lb_$i` 与 `--env ld_$i --event-template "...|lockdown"`）。
