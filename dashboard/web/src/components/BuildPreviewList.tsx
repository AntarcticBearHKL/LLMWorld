  ...
  return (
    <li className="flex min-w-0 items-center gap-2">
      <span
        ...
        aria-hidden
      />
      <span className="num min-w-0 flex-1 truncate t-caption text-fg-muted" title={path}>
        {path}
      </span>
...
export function BuildPreviewList({ preview, isPending, error, needsHouse, house }: BuildPreviewListProps) {
  if (needsHouse && house.length === 0) {
    return <p className="t-caption text-fg-subtle">Pick a target household to view read/write paths.</p>
  }

  if (isPending) {
    return <p className="t-caption text-fg-subtle">Loading read/write preview…</p>
  }

  if (error !== null) {
    return <p className="t-caption text-danger">Failed to load read/write preview: {error}</p>
  }

  if (preview === undefined) {
    return <p className="t-caption text-fg-subtle">No preview.</p>
  }
  ...
  return (
    <div className="flex flex-col gap-2">
      <div className="flex flex-wrap items-baseline gap-2">
        ...
        {preview.house !== null ? (
          <span className="num ml-auto t-caption text-fg-subtle" title="Target household resolved by the backend">
            {preview.house}
          </span>
        ) : null}
      </div>
      ...
      {empty ? (
        <p className="t-caption text-fg-subtle">(backend returned no read/write paths)</p>
      ) : (
        <>
            ...
            <Group title="overwrite" hint="backs up first" tone="energy">
              <ul className="flex flex-col gap-1">
                {preview.overwrites.map((path) => (
                  <li key={path} className="num min-w-0 truncate t-caption text-energy" title={path}>
                    {path}
                  </li>
                ))}
              </ul>
