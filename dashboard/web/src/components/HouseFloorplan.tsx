...
export function HouseFloorplan({ replay, className }: HouseFloorplanProps) {
  ...
  return (
    <Panel
      ...
      actions={<span className="num t-body text-energy">{formatWatts(houseWatts)}</span>}
    >
      {replay === undefined ? (
        <div className="flex flex-1 items-center justify-center p-4">
          <span className="t-caption">Loading floor plan…</span>
        </div>
      ) : (
        <>
                ...
                >
                  <div className="flex items-baseline justify-between gap-1.5">
                    <span
                      className={cn(
                        "truncate",
                        occupied ? "t-body text-fg" : "t-caption text-fg-muted",
                      )}
                    >
                      {room}
                    </span>
                    {watts > 0 ? (
                      <span className="num shrink-0 t-body text-energy">
                        {formatWatts(watts)}
                      </span>
                    ) : null}
                  </div>
                  ...
                  <div className="mt-1 flex flex-wrap items-center gap-1">
                    {here.map((member) => (
                      <span
                        key={member}
                        title={member}
                        className="num t-micro flex size-5 items-center justify-center rounded-full"
                        style={{
          ...
          <div className="mt-auto flex shrink-0 flex-wrap items-center gap-x-4 gap-y-1 border-t border-border px-3 py-2">
            ...
            <span className="t-micro ml-auto">Powered appliances {activeCount}</span>
          </div>
        </>
      )}
    </Panel>
